  ...
  renderDialog = () => {
    ...
    return (
      ...
      >
        ...
        <p>
          <a id="source-link" href={tile.href}>
            Source
          </a>
        </p>
      </Modal>
...
const ProjGridList = memo(props => {
  const { classes, style, windowWidth } = props;
  const numCols = windowWidth > 800 ? 3 : windowWidth > 400 ? 2 : 1;

  return (
    <div className={classes.root}>
      <GridList className={classes.gridList} cellHeight={"auto"} cols={numCols}>
        {tileData.map(tile => {
          return (
            <ProjDialog
              key={tile.title}
